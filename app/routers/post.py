...
my_posts = [{'title': 'Title 1', 'content': 'Content 1', 'id': 1}, {'title': 'Title2', 'content': 'Content 2', 'id': 2}]
...
async def get_posts(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user),
    ...
    #?limit=1&skip=0&search=Beach%20p
...
@router.post('/createpost2')
async def create_post2(payload: dict = Body(...)):
    return {"new_post": f"Post created. Title: {payload['title']}, content: {payload['content']}"}
